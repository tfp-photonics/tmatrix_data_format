...
def h5save(c: Config, t, file=None, name="t_data", description=""):
    """
    Store function complying with the requiments specified in the T-matrix data format project
    """
    ...
    with h5py.File(c.path_output + f"{name}.tmat.h5", "w") as f:
        ...
        f["computation/mesh"].create_dataset(
            "eps_grid", data=eps_grid, compression="gzip"
        )
        f["computation/mesh"].attrs[
            "description"
        ] = "eps_grid is the voxel grid of permittivity distribution inside a box of sidelength defined by the parameter domain in the method_parameters. This does not include the PML."
        ...
        f["computation/files/source_script.py"] = file
