...
def get_sca_coef(
    ...
    def rotate_ndarray_last_axis(arr: ArrayLike):
        return np.tensordot(arr, rotation_data.rotation_matrix, axes=([-1], [-1]))
    ...
    def get_sc_coef_via_integral(vswf_a, vswf_b):
        integrand = np.cross(rot_E_sc_rot, vswf_a) - k[
            :, None, None, None, None
        ] * np.cross(vswf_b, sim_res_rot.e)
...
def get_sca_coefs(c: Config, rotation_data: RotationData, sim_res: SimData) -> NDArray:
    ...
    for l in range(1, c.l_max + 1):
        for m in range(-l, l + 1):
            ...
            sca_coefs = np.concatenate((sca_coefs, ab)) if sca_coefs.size else ab
    return sca_coefs.T
