...
class Config(PersitableDataClass):
    ...
    keywords : list[str] | str = 'passive, reciprocal'
    material_names: list[str] = field(default_factory=lambda:['', ''])
    material_descriptions: list[str] = field(default_factory=lambda:['', ''])
    material_keywords: list[str] = field(default_factory=lambda:['', ''])
    mu: list[float] = field(default_factory=lambda:[1., 1.])
    tol: float = 1e-11
    runtime: float = 150
    ...
    def scatter_sim_params_hash(self) -> str:
        relevant_information = str.encode(
            f"{self.resolution}{self.eps_embedding}{self.material}{self.object_size}{self.dpml}{self.f_src}{self.df_src}{self.n_freq}"
        )
