...
###Parameters
################################################################################
lambdas = np.linspace(400, 1000, 121)  # steps of 5 nm
name = "Tmatrix_sphere_CdS"
shape = "sphere"
radius = 150
params = {"radius": radius}
eps_sph = 2.5**2
eps_emb = 1.0
material_names = ["Air, Vacuum", "CdS, Cadmium Sulfide"]
material_descriptions = ["", ""]
material_keywords = ["nondispersive", "nondispersive, semiconductor"]
material_values = [eps_emb, eps_sph]
mu = [1.0, 1.0]
lmax = 3
ms = 12.0  # 12.0
...
c.setNumber("1Geometry/01ellipsoid X-radius [nm]", value=radius)
c.setNumber("1Geometry/02ellipsoid Y-radius [nm]", value=radius)
c.setNumber("1Geometry/03ellipsoid Z-radius [nm]", value=radius)
c.setNumber(
...
with h5py.File("sphere_here.tmat.h5", "w") as fobj:
    ...
    tools.base_data(
        ...
        tmatrices=tmats,
        name="Sphere",
        description=(f"A simple sphere with {radius} nm radius"),
    ...
    tools.computation_data(
        ...
        keys={"mesh_size": ms},
        software=f"onelab=1.0, python={sys.version.split()[0]}, numpy={np.__version__}, h5py={h5py.__version__}",
    ...
    fobj["computation/mesh.msh"].attrs["unit"] = "nm"
