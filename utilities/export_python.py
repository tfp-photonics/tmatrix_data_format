...
with h5py.File(f, "w") as f:
    ...
    sca_mat["relative_permittivity"] = np.ones(len(wavelength), dtype=complex) * (
        -11.4 + 1.181j
    )
    ...
    f["computation"].attrs[
        "software"
    ] = f"SMARTIES=1.1, python={sys.version.split()[0]}, h5py={h5py.__version__}"
    f["computation"].attrs["name"] = "SMARTIES"
    f.attrs["name"] = "Au prolate spheroid in water"
    f.attrs["keywords"] = (
        "gold, spheroid, ebcm, passive, reciprocal, czinfinity, mirrorxyz"
    )
    f.attrs["description"] = (
        "Computation using SMARTIES, a numerically robust EBCM implementation for spheroids"
    )
